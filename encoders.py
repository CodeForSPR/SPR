    ...
    def forward(self, nodes):
        if self.last_layer:
            return self.aggregator.forward(
                nodes, [self.train_pos[node] for node in nodes], [self.train_neg[node] for node in nodes])
        else:
            return self.aggregator.forward(
                nodes, [self.train_pos[node.item()] for node in nodes], [self.train_neg[node.item()] for node in nodes])
